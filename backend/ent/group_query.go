...
func (gq *GroupQuery) QuerySubmits() *SubmitQuery {
	query := (&SubmitClient{config: gq.config}).Query()
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withSubmits: gq.withSubmits.Clone(),
...
func (gq *GroupQuery) WithSubmits(opts ...func(*SubmitQuery)) *GroupQuery {
	query := (&SubmitClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withSubmits = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withSubmits; query != nil {
		if err := gq.loadSubmits(ctx, query, nodes,
			func(n *Group) { n.Edges.Submits = []*Submit{} },
			func(n *Group, e *Submit) { n.Edges.Submits = append(n.Edges.Submits, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
