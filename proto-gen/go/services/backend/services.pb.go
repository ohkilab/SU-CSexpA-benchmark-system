// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.31.0
// 	protoc        (unknown)
// source: services/backend/services.proto

package backend

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_services_backend_services_proto protoreflect.FileDescriptor

var file_services_backend_services_proto_rawDesc = []byte{
	,
}

var file_services_backend_services_proto_goTypes = []interface{}{
	(*PostSubmitRequest)(nil),               // 0: backend.PostSubmitRequest
	(*GetSubmitRequest)(nil),                // 1: backend.GetSubmitRequest
	(*ListSubmitsRequest)(nil),              // 2: backend.ListSubmitsRequest
	(*GetContestantInfoRequest)(nil),        // 3: backend.GetContestantInfoRequest
	(*ListContestsRequest)(nil),             // 4: backend.ListContestsRequest
	(*GetContestRequest)(nil),               // 5: backend.GetContestRequest
	(*GetRankingRequest)(nil),               // 6: backend.GetRankingRequest
	(*VerifyTokenRequest)(nil),              // 7: backend.VerifyTokenRequest
	(*PostLoginRequest)(nil),                // 8: backend.PostLoginRequest
	(*PingUnaryRequest)(nil),                // 9: backend.PingUnaryRequest
	(*PingServerSideStreamingRequest)(nil),  // 10: backend.PingServerSideStreamingRequest
	(*CreateContestRequest)(nil),            // 11: backend.CreateContestRequest
	(*UpdateContestRequest)(nil),            // 12: backend.UpdateContestRequest
	(*DeleteContestRequest)(nil),            // 13: backend.DeleteContestRequest
	(*CreateGroupsRequest)(nil),             // 14: backend.CreateGroupsRequest
	(*ListGroupsRequest)(nil),               // 15: backend.ListGroupsRequest
	(*DeleteGroupRequest)(nil),              // 16: backend.DeleteGroupRequest
	(*PostSubmitResponse)(nil),              // 17: backend.PostSubmitResponse
	(*GetSubmitResponse)(nil),               // 18: backend.GetSubmitResponse
	(*ListSubmitsResponse)(nil),             // 19: backend.ListSubmitsResponse
	(*GetContestantInfoResponse)(nil),       // 20: backend.GetContestantInfoResponse
	(*ListContestsResponse)(nil),            // 21: backend.ListContestsResponse
	(*GetContestResponse)(nil),              // 22: backend.GetContestResponse
	(*GetRankingResponse)(nil),              // 23: backend.GetRankingResponse
	(*VerifyTokenResponse)(nil),             // 24: backend.VerifyTokenResponse
	(*PostLoginResponse)(nil),               // 25: backend.PostLoginResponse
	(*PingUnaryResponse)(nil),               // 26: backend.PingUnaryResponse
	(*PingServerSideStreamingResponse)(nil), // 27: backend.PingServerSideStreamingResponse
	(*CreateContestResponse)(nil),           // 28: backend.CreateContestResponse
	(*UpdateContestResponse)(nil),           // 29: backend.UpdateContestResponse
	(*DeleteContestResponse)(nil),           // 30: backend.DeleteContestResponse
	(*CreateGroupsResponse)(nil),            // 31: backend.CreateGroupsResponse
	(*ListGroupsResponse)(nil),              // 32: backend.ListGroupsResponse
	(*DeleteGroupResponse)(nil),             // 33: backend.DeleteGroupResponse
}
var file_services_backend_services_proto_depIdxs = []int32{
	0,  // 0: backend.BackendService.PostSubmit:input_type -> backend.PostSubmitRequest
	1,  // 1: backend.BackendService.GetSubmit:input_type -> backend.GetSubmitRequest
	2,  // 2: backend.BackendService.ListSubmits:input_type -> backend.ListSubmitsRequest
	3,  // 3: backend.BackendService.GetContestantInfo:input_type -> backend.GetContestantInfoRequest
	4,  // 4: backend.BackendService.ListContests:input_type -> backend.ListContestsRequest
	5,  // 5: backend.BackendService.GetContest:input_type -> backend.GetContestRequest
	6,  // 6: backend.BackendService.GetRanking:input_type -> backend.GetRankingRequest
	7,  // 7: backend.BackendService.VerifyToken:input_type -> backend.VerifyTokenRequest
	8,  // 8: backend.BackendService.PostLogin:input_type -> backend.PostLoginRequest
	9,  // 9: backend.HealthcheckService.PingUnary:input_type -> backend.PingUnaryRequest
	10, // 10: backend.HealthcheckService.PingServerSideStreaming:input_type -> backend.PingServerSideStreamingRequest
	11, // 11: backend.AdminService.CreateContest:input_type -> backend.CreateContestRequest
	12, // 12: backend.AdminService.UpdateContest:input_type -> backend.UpdateContestRequest
	13, // 13: backend.AdminService.DeleteContest:input_type -> backend.DeleteContestRequest
	14, // 14: backend.AdminService.CreateGroups:input_type -> backend.CreateGroupsRequest
	15, // 15: backend.AdminService.ListGroups:input_type -> backend.ListGroupsRequest
	16, // 16: backend.AdminService.DeleteGroup:input_type -> backend.DeleteGroupRequest
	17, // 17: backend.BackendService.PostSubmit:output_type -> backend.PostSubmitResponse
	18, // 18: backend.BackendService.GetSubmit:output_type -> backend.GetSubmitResponse
	19, // 19: backend.BackendService.ListSubmits:output_type -> backend.ListSubmitsResponse
	20, // 20: backend.BackendService.GetContestantInfo:output_type -> backend.GetContestantInfoResponse
	21, // 21: backend.BackendService.ListContests:output_type -> backend.ListContestsResponse
	22, // 22: backend.BackendService.GetContest:output_type -> backend.GetContestResponse
	23, // 23: backend.BackendService.GetRanking:output_type -> backend.GetRankingResponse
	24, // 24: backend.BackendService.VerifyToken:output_type -> backend.VerifyTokenResponse
	25, // 25: backend.BackendService.PostLogin:output_type -> backend.PostLoginResponse
	26, // 26: backend.HealthcheckService.PingUnary:output_type -> backend.PingUnaryResponse
	27, // 27: backend.HealthcheckService.PingServerSideStreaming:output_type -> backend.PingServerSideStreamingResponse
	28, // 28: backend.AdminService.CreateContest:output_type -> backend.CreateContestResponse
	29, // 29: backend.AdminService.UpdateContest:output_type -> backend.UpdateContestResponse
	30, // 30: backend.AdminService.DeleteContest:output_type -> backend.DeleteContestResponse
	31, // 31: backend.AdminService.CreateGroups:output_type -> backend.CreateGroupsResponse
	32, // 32: backend.AdminService.ListGroups:output_type -> backend.ListGroupsResponse
	33, // 33: backend.AdminService.DeleteGroup:output_type -> backend.DeleteGroupResponse
	17, // [17:34] is the sub-list for method output_type
	0,  // [0:17] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_services_backend_services_proto_init() }
func file_services_backend_services_proto_init() {
	if File_services_backend_services_proto != nil {
		return
	}
	file_services_backend_messages_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_services_backend_services_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   3,
		},
		GoTypes:           file_services_backend_services_proto_goTypes,
		DependencyIndexes: file_services_backend_services_proto_depIdxs,
	}.Build()
	File_services_backend_services_proto = out.File
	file_services_backend_services_proto_rawDesc = nil
	file_services_backend_services_proto_goTypes = nil
	file_services_backend_services_proto_depIdxs = nil
}
